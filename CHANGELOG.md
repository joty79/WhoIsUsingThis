...
- Bumped `app-metadata.json` to `1.0.6` and removed the folder-background `System Tools > Windows > Who is using this?` entry as well; the visible desktop right-click menu uses `Directory\Background`, not only `DesktopBackground`.
